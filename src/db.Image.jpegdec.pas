...
(*
    ...
    Copyright (C) 2017 Mark Griffiths
  ...
  *** BEGIN LICENSE BLOCK *****
  ...
  Contributor(s):
    March 2010: Arnaud Bouchez for Delphi integration
      http://synopse.info
    2017: Mark Griffiths made the code fully thread safe.
      http://www.marktg.com/jpegdec
  ...
  ***** END LICENSE BLOCK *****
  ...
  Version 1.18
   - Removed use of a global variable to allow full multithreaded use of
     this library - thanks Mark Griffiths for the patch!
...
{$A-}
  ...
  // - must be allocated by JpegDecode() function, not on stack nor heap
  ...
  TJpegDecode = object
    ...
    procedure ToBMI(out BMI: TBitmapInfo);
    ...
    /// write the resulting image into to a Win32 bitmap
    // - the bitmap will be resized to match expected width/height, if necessary
    // - return false on error
    // - you can then call TJpegDecode.Free to release the decoded memory
    function ToBitmap(bmp: TBitmap): boolean; overload;
...
{$A+}
  /// reference pointer to a jpeg picture content, as returned by JpegDecode()
...
procedure TJpegDecode.ToBMI(out BMI: TBitmapInfo);
begin
...
begin
  if @self=nil then
    ...
    if not ToBitmap(result) then
      FreeAndNil(result);
  end;
end;

function TJpegDecode.ToBitmap(bmp: TBitmap): boolean;
var BMI: TBitmapInfo;
    DC: HDC;
begin
  result := false;
  ...
  ToBMI(BMI);
  DC := GetDC(0);
  result := SetDIBits(DC,bmp.Handle,0,height,pRGB,BMI,DIB_RGB_COLORS)=height;
  ReleaseDC(0,DC);
end;
